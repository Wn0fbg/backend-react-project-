...
export const isAuthenticated = catchAsyncError(async (req, res, next) => {
  // ИСПРАВЛЕНО: правильно получаем токен из cookies
  ...
  // Поиск токена в таблице token_store
  ...
  // ИСПРАВЛЕНО: user.rows[0] вместо user.row[0]
...
export const authorizeRoles = (...roles) => {
  return (req, res, next) => {
    ...
    next(); // ВАЖНО: не забывай вызывать next()
  };
};
